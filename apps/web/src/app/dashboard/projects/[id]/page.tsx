    ...
    <div className="min-h-screen bg-black p-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-mono text-white/40 uppercase tracking-wider">Project Overview</span>
            </div>
            <h1 className="text-xl font-mono font-bold text-white">{project.name}</h1>
            <p className="text-sm font-mono text-white/50 mt-1 max-w-2xl">{project.description || 'No description'}</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
        <div className="bg-white/5 border border-white/10 p-4">
          <p className="text-[10px] font-mono text-white/40 uppercase mb-1">Domain</p>
          <p className="text-sm font-mono text-white font-bold">{project.domain}</p>
        </div>
        <div className="bg-white/5 border border-white/10 p-4">
          <p className="text-[10px] font-mono text-white/40 uppercase mb-1">Prediction Core</p>
          <p className="text-sm font-mono text-cyan-400 font-bold flex items-center gap-1">
            <Target className="w-3 h-3" />
            Collective Dynamics
          </p>
        </div>
        <div className="bg-white/5 border border-white/10 p-4">
          <p className="text-[10px] font-mono text-white/40 uppercase mb-1">Default Horizon</p>
          <p className="text-sm font-mono text-white font-bold">{project.settings?.default_horizon || 100} ticks</p>
        </div>
        <div className="bg-white/5 border border-white/10 p-4">
          <p className="text-[10px] font-mono text-white/40 uppercase mb-1">Last Updated</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          {hasBaseline && rootNode ? (
            <div className="space-y-3">
              ...
              <Link href={`/dashboard/nodes/${rootNode.node_id}`}>
                <Button variant="outline" size="sm" className="w-full mt-4 font-mono text-[10px] border-white/20 text-white/60 hover:bg-white/5">
                  VIEW BASELINE NODE
                </Button>
              </Link>
            </div>
            ...
            <div className="space-y-3">
              <p className="text-xs font-mono text-white/50">
                Run a baseline simulation to establish the default future before exploring what-if scenarios.
              </p>
              <Button
                onClick={handleRunBaseline}
                disabled={isCreatingBaseline}
                className="w-full bg-yellow-500/20 hover:bg-yellow-500/30 border-yellow-500/50 text-yellow-400"
              >
                {isCreatingBaseline ? (
        ...
        <div className="bg-white/5 border border-white/10 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-mono font-bold text-white text-sm uppercase">Latest Node</h3>
          </div>
          ...
          {nodesLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs font-mono text-white/40">No nodes yet</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white/5 border border-white/10 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-mono font-bold text-white text-sm uppercase">Reliability</h3>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-8">
        <div className="bg-white/5 border border-white/10 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-mono text-white/40 uppercase">Nodes</p>
              <p className="text-xl font-mono font-bold text-white">{stats?.node_count || project.node_count || 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white/5 border border-white/10 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-mono text-white/40 uppercase">Runs</p>
              <p className="text-xl font-mono font-bold text-white">{stats?.run_count || project.run_count || 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white/5 border border-white/10 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-mono font-bold text-white">{stats?.completed_runs || 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white/5 border border-white/10 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-mono font-bold text-white">{project.settings?.default_agent_count?.toLocaleString() || '—'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Suggested Actions */}
      <div className="mb-8">
        <h2 className="text-sm font-mono font-bold text-white mb-4 uppercase flex items-center gap-2">
          <Zap className="w-4 h-4 text-yellow-400" />
          Suggested Actions
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <Link href={`/dashboard/nodes?project_id=${projectId}`}>
            <div className="bg-white/5 border border-white/10 p-4 hover:bg-white/[0.07] hover:border-cyan-500/30 transition-all cursor-pointer group">
              <div className="flex items-center gap-3">
                <Map className="w-5 h-5 text-cyan-400" />
                <div>
                  <p className="text-sm font-mono text-white font-bold group-hover:text-cyan-400 transition-colors">Open Universe Map</p>
                  <p className="text-[10px] font-mono text-white/40">Explore future branches</p>
                </div>
              </div>
            </div>
          </Link>
          ...
          <button
            ...
            className={cn(
              "bg-white/5 border border-white/10 p-4 hover:bg-white/[0.07] hover:border-purple-500/30 transition-all cursor-pointer group text-left",
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-mono text-white font-bold group-hover:text-purple-400 transition-colors">Ask a What-If</p>
                <p className="text-[10px] font-mono text-white/40">{hasBaseline ? "Create new scenarios" : "Run baseline first"}</p>
              </div>
            </div>
          </button>

          <Link href={`/dashboard/personas?project_id=${projectId}`}>
            <div className="bg-white/5 border border-white/10 p-4 hover:bg-white/[0.07] hover:border-green-500/30 transition-all cursor-pointer group">
              <div className="flex items-center gap-3">
                <Users className="w-5 h-5 text-green-400" />
                <div>
                  <p className="text-sm font-mono text-white font-bold group-hover:text-green-400 transition-colors">Manage Personas</p>
                  <p className="text-[10px] font-mono text-white/40">Add or edit personas</p>
                </div>
              </div>
            </div>
          </Link>
          ...
          <Link href={`/dashboard/projects/${projectId}/calibrate`}>
            <div className="bg-white/5 border border-white/10 p-4 hover:bg-white/[0.07] hover:border-yellow-500/30 transition-all cursor-pointer group">
              <div className="flex items-center gap-3">
                <TrendingUp className="w-5 h-5 text-yellow-400" />
                <div>
                  <p className="text-sm font-mono text-white font-bold group-hover:text-yellow-400 transition-colors">Calibrate</p>
                  <p className="text-[10px] font-mono text-white/40">Improve accuracy</p>
                </div>
              </div>
            </div>
          </Link>

      ...
      <div className="w-64 border-r border-white/10 flex flex-col">
        ...
        <div className="p-4 border-b border-white/10">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-mono text-white/40 uppercase tracking-wider">Project Personas</span>
          </div>
        ...
        <div className="bg-white/5 p-3 space-y-2">
          {Object.entries(demographics).slice(0, 8).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between text-xs font-mono">
              ...
              <span className="text-white truncate max-w-[120px]">{String(value)}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div>
          ...
          <div className="bg-white/5 p-3 space-y-2">
            {Object.entries(psychographics).slice(0, 5).map(([key, value]) => (
              <div key={key} className="flex items-center justify-between text-xs font-mono">
                <span className="text-white/40 capitalize">{key.replace(/_/g, ' ')}</span>
                <span className="text-white truncate max-w-[120px]">{typeof value === 'object' ? JSON.stringify(value) : String(value)}</span>
              </div>
            ))}
          </div>
        </div>

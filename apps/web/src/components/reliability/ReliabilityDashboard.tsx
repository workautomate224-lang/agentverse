    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">Reliability Report</h1>
            <p className="text-sm text-white/60">
              Generated {new Date(report.generated_at).toLocaleDateString()} | Valid until{' '}
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-2 px-4 py-2 border border-cyan-500/30',
            'bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20',
            'disabled:opacity-50 disabled:cursor-not-allowed'
          )}
        >
          <RefreshCw className={cn('w-4 h-4', isRefreshing && 'animate-spin')} />
          {isRefreshing ? 'Computing...' : 'Refresh Report'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Main Score Ring */}
        <div className="lg:col-span-1 border border-white/10 bg-black/40 p-6 flex flex-col items-center justify-center">
          <ScoreRing score={report.overall_reliability_score} size="lg" />
          <div className="mt-4 text-center">
            <h2 className="text-lg font-semibold">Overall Reliability</h2>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="border border-white/10 bg-black/40 p-4">
            ...
            <div className="text-xs text-white/40 mt-1">{report.stability.seeds_tested} seeds tested</div>
          </div>
          ...
          <div className="border border-white/10 bg-black/40 p-4">
            ...
            <div className="text-xs text-white/40 mt-1">{report.data_gaps.critical_gaps.length} critical gaps</div>
          </div>
        </div>
      </div>
            ...
            <div className="border border-cyan-500/30 bg-cyan-500/10 p-4">
              ...
              <ul className="space-y-2">
                {report.recommendations.map((rec, i) => (
                  <li key={i} className="text-sm text-cyan-300/80 flex items-start gap-2">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-sm text-white/60">Best: {Math.round(report.calibration.best_scenario_accuracy * 100)}%</div>
                <div className="text-sm text-white/60">Worst: {Math.round(report.calibration.worst_scenario_accuracy * 100)}%</div>
                <div className="text-sm text-white/60">Mean Error: {report.calibration.mean_error.toFixed(3)}</div>
              </div>
            </div>
            <div className="border-t border-white/10 pt-3">
              ...
              <button className="text-xs text-cyan-400 hover:text-cyan-300">View scenario details &rarr;</button>
            </div>
          </div>
        </SectionCard>
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-sm text-white/60">Seeds tested: {report.stability.seeds_tested}</div>
            ...
            <div className="border-t border-white/10 pt-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-green-400 font-mono">{report.stability.most_stable_outcome}</span>
              </div>
              <div className="flex justify-between text-sm mt-1">
                <span className="text-white/60">Least stable:</span>
                <span className="text-yellow-400 font-mono">{report.stability.least_stable_outcome}</span>
              </div>
            </div>
          </div>
        </SectionCard>
              ...
              {report.sensitivity.top_impact_variables.map((variable) => {
                ...
                return (
                  <div key={variable} className="flex items-center gap-3">
                    <span className="text-sm font-mono text-white/80 w-32 truncate">{variable}</span>
                    ...
                    <span className="text-sm text-white/60 w-12 text-right">{Math.round(score * 100)}%</span>
                  </div>
                );
              })}
            </div>
        ...
        >
          <div className="space-y-4">
            ...
            {report.drift.drifted_variables.length > 0 && (
              <div className="border-t border-white/10 pt-3">
                <div className="text-sm text-white/60 mb-2">Drifted variables:</div>
                <div className="flex flex-wrap gap-2">
                  {report.drift.drifted_variables.map((v) => (
                    <span key={v} className="px-2 py-1 text-xs bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">
                      {v}
                    </span>
                  ))}
                </div>
            ...
            {report.drift.recommendations && (
              <div className="text-sm text-white/60">{report.drift.recommendations[0]}</div>
            )}
          </div>
        </SectionCard>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Object.entries(report.confidence.by_time_horizon).map(([horizon, score]) => (
            <div key={horizon} className="text-center">
              ...
              <div className="mt-2 text-sm text-white/60">{horizon.replace('_', ' ')}</div>
            </div>
          ))}
        </div>
      </SectionCard>
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-2xl font-mono font-bold text-cyan-400">{report.data_gaps.total_variables}</div>
              <div className="text-xs text-white/60">Total Variables</div>
            </div>
            <div>
              <div className="text-2xl font-mono font-bold text-yellow-400">{report.data_gaps.variables_with_gaps}</div>
              <div className="text-xs text-white/60">With Gaps</div>
            </div>
            <div>
              <div className="text-2xl font-mono font-bold text-red-400">{report.data_gaps.critical_gaps.length}</div>
              <div className="text-xs text-white/60">Critical Gaps</div>
            </div>
          </div>
          {report.data_gaps.recommendations.length > 0 && (
            <div className="border-t border-white/10 pt-3">
              <div className="text-sm text-white/60">{report.data_gaps.recommendations[0]}</div>
            </div>
          )}
        </div>
      </SectionCard>
      ...
      <div className="text-center text-xs text-white/40 pt-4 border-t border-white/10">
        Engine v{report.engine_version} | Report v{report.report_version} | Generated{' '}
